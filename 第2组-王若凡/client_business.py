...
def flush_info():
    ...
    get_info = {"id":user_id}
    packet_get_info = get_pack("c_s", "business", "refresh_order", content=str(get_info),
    ...
    packet_print_window.send_show("c_s", "business", "refresh_order", len(packet_get_info), content=str(get_info),
                               sessionkey=sessionkey_global, privatekey=privatekey_global)
    if ret_send == -2:
    ...
    # print(ret_list_content)
    packet_print_window.input_show(str(ret_list_content))
    # recv_inf ('商家id', '店名', '地址', '电话号码', '营业额', '状态')。 ('12111', '老乡鸡', '朝阳路1号', '18088886666', 135, '工作')
    # recv_order ('订单id', '商家名', '金额', '状态', '日期')
    # recv_menu ('订单id', '顾客姓名', '顾客电话', '订单金额')
...
def submit_order():
    ...
    for _num in order_list:
        ...
        send_order.append(cur_menu[_num][0])

    packet_send_order = get_pack("c_s", "business", "finish_order", content=str(send_order),
                                 sessionkey=sessionkey_global, privatekey=privatekey_global)
    packet_print_window.send_show("c_s", "business", "finish_order", len(packet_send_order), content=str(send_order),
                                 sessionkey=sessionkey_global, privatekey=privatekey_global)
...
def change_work_state():
    ...
    packet_change_req = get_pack('c_s', 'business', 'one_click_commuting', content=str(change_req),
                            sessionkey=sessionkey_global, privatekey=privatekey_global)
    packet_print_window.send_show('c_s', 'business', 'one_click_commuting', len(packet_change_req), content=str(change_req),
                            sessionkey=sessionkey_global, privatekey=privatekey_global)
...
def alter_info():  # 修改个人信息
    def commit_info_change():
        ...
        alter_info = {"id": user_id, "name": altered_name, "address": altered_addr, "phonenumber": altered_tel}
        packet_alter_info = get_pack("c_s",'business', 'modify_info', content=str(alter_info),
                                sessionkey=sessionkey_global, privatekey=privatekey_global)
        packet_print_window.send_show("c_s",'business', 'modify_info', len(packet_alter_info), content=str(alter_info),
                                sessionkey=sessionkey_global, privatekey=privatekey_global)
        ...
        '''if int(ret.state) != 1:
            tk.messagebox.showinfo(title='错误', message='商家信息修改失败')
            return
        else:
            tk.messagebox.showinfo(title='成功', message='商家信息修改成功')
    ...
    # info = ['1001', 'Cheney', '南望山', '18055264512', 9826359]
    ...
    tk.Label(window_alter_info, text=recv_inf[0] + ' ' * 30 + '不可更改').place(x=110, y=25)
    ...
    tk.Label(window_alter_info, text=recv_inf[1]).place(x=110, y=80)
    ...
    tk.Label(window_alter_info, text='地址').place(x=10, y=135)
    tk.Label(window_alter_info, text=recv_inf[2]).place(x=110, y=135)
    tk.Entry(window_alter_info, textvariable=alter_new_addr).place(x=250, y=135)

    #
    alter_new_tel = tk.StringVar()
    tk.Label(window_alter_info, text='号码').place(x=10, y=190)
    tk.Label(window_alter_info, text=recv_inf[3]).place(x=110, y=190)
...
def charge_money():
    def charge_money_out():
        ...
        print("密码hash", type(hash.encrypt(user_pwd)), hash.encrypt(user_pwd))
        charge_money = {"id": user_id, "passwd": hash.encrypt(user_pwd), "money": money}

        packet_charge_money = get_pack("c_s", "business", "cash_out", content=str(charge_money),
                                       sessionkey=sessionkey_global, privatekey=privatekey_global)
        packet_print_window.send_show("c_s", "business", "cash_out", len(packet_charge_money), content=str(charge_money),
                                       sessionkey=sessionkey_global, privatekey=privatekey_global)
...
style_main = Style(theme='sandstone')
...
# 标签 用户名密码
tk.Label(main_window, text='商店名:').place(x=left_index, y=15)
...
class My_Tk():
    ...
    def create_button(self):
        Button(self.main_window, text='刷新待订单出餐', command=self.insert_tv).pack()
    ...
    def create_heading(self, ):
        ...
        self.columns = ['订单号', '收货人姓名', '收件人电话', '金额']
...
loggin_window.title('外卖管理_商家登录界面 1.0 ')
...
style_log = Style(theme='sandstone')
...
def log_in(id1, password1) -> socket.socket:
    ...
    while True:
        ...
        msg2as = get_pack('c_a', 'business', 'get_ticket1', content=str(info2as))  # 改成str
        packet_print_window.send_show('c_a', 'business', 'get_ticket1', len(msg2as), content=str(info2as))
        # a = pack(msg2as)
    ...
    while True:
        ...
        packet_print_window.send_show('c_t', 'business', 'request_ticket2', len(msg2tgs), content=str(info2tgs))
        # s_tgs.(msg2tgs)
    ...
    while True:
        ...
        packet_print_window.send_show('c_s', sign='business', control_type='message5', total_length=len(msg2ss), content=str(info2ss))
...
def usr_log_in():
    ...
    if type(s_ss) == type(socket.socket()):  # 如果登陆成功
        ...
        global user_pwd
        global en_user_pwd
        en_user_pwd = hash.encrypt(user_pwd)
        global sessionkey_global
        global privatekey_global
        global publickey_global
        user_id = usr_name
...
def usr_sign_up():
    ...
    def signtowcg():
        ...
        if '' in [n_id, n_pwd, n_pwd_c, n_name, n_add, n_tel]:
            ...
            print("注册时的密码哈希", type(n_pwd_en), n_pwd_en)
        ...
        packet_print_window.send_show('c_a', 'business', 'request_enroll', len(msg2as), content=str(info2as))
        s_as.send(msg2as)
        ...
        # 开始连接SS服务器
        s_ss.connect(address_ss)
        info2ss = {"id": n_id, "passwd": n_pwd_en, "name": n_name, "address": n_add,
                   "phonenumber": n_tel}
        msg2ss = get_pack('c_s', 'business', 'signature', content=str(info2ss))  # 请求注册的消息
        packet_print_window.send_show('c_s', 'business', 'signature', len(msg2ss), content=str(info2ss))
        # print(msg2ss)
    ...
    tk.Label(window_sign_up, text='商店名').place(x=10, y=130)
    tk.Entry(window_sign_up, textvariable=new_name).place(x=150, y=130)
    # 地址
    new_add = tk.StringVar()
    tk.Label(window_sign_up, text='地址').place(x=10, y=170)
    tk.Entry(window_sign_up, textvariable=new_add).place(x=150, y=170)
    # 电话
...
def usr_mod_passwd():
    def ok_alt():
        ...
        packet_print_window.send_show('c_a', 'business', 'modify_pwd', len(msg2as), content=str(info2as))
        s_as.send(msg2as)
        flag_as = s_as.recv(buffer_size)
        msg_re = pack(flag_as)
        ...
        packet_print_window.send_show('c_s', 'business', 'modify_pwd', len(msg2ss), content=str(info2ss))
        s_ss.send(msg2ss)
