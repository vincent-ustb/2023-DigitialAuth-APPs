...
def func_LSB_suijijiange_yinxie(str1, str2, str3):
    ...
    # 获取需要隐藏的信息
    global keylen
    key = get_key(str2)  # 假设get_key是之前定义的函数
    keylen = len(key)
    print(key)
    print(keylen)
    ...
    for count in range(keylen):
        w, h = q_converto_wh(q)  # 假设 q_converto_wh 是之前定义的函数
        pixel = im.getpixel((w, h))
        pixel = pixel - mod(pixel, 2) + int(key[count])  # 假设 mod 是之前定义的函数
...
def func_LSB_quyujiaoyan_yinxie(str1, str2, str3):
    ...
    if width * height < size * keylen:
        tkinter.messagebox.showinfo('提示',
                                    'size设置过大，请重新设置，size最大值为: ' + str(int(width * height / keylen)))
...
# 顶部标题
Label(top_frame, text="基于数字图像的可视化水印系统", font=fontStyle).pack(side=TOP, pady=20)

# 左侧面板
ttk.Label(left_frame, text="空间域水印", font=fontStyle).pack(pady=10)
ttk.Button(left_frame, text="LSB基本算法", command=create_LSB_basic).pack(pady=10)
ttk.Button(left_frame, text="LSB算法改进", command=create_LSB_improve).pack(pady=10)
ttk.Button(left_frame, text="图像降级算法及其改进", command=create_image_downgrade).pack(pady=10)

# 右侧面板
ttk.Label(right_frame, text="变换域水印", font=fontStyle).pack(pady=10)
ttk.Button(right_frame, text="DCT水印", command=create_DCT).pack(pady=10)


# 设置主题样式
style = ttk.Style()
style.theme_use('clam')  # 使用主题风格
