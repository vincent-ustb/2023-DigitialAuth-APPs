...
class ChatSocket:
    ...
    def __init__(self):
        ...
        # 设置验证模式为 CERT_REQUIRED，在此模式下，需要验证服务器证书
        context.verify_mode = ssl.CERT_REQUIRED                    #new
        # 加载用于验证服务器证书的 CA 证书
        context.load_verify_locations("E:\\third\\Digital authentication\\test\\pythonProject-main\\2\\pythonProject-main\\python_chat\\ca.crt")  # new
        # 创建对象的同时，会创建连接服务器的socket
        self.ssl_socket = socket.socket()  # 创建socket
        self.client_socket = context.wrap_socket(self.ssl_socket, server_hostname='127.0.0.1')    # new
        self.client_socket.connect(('127.0.0.1', 5000))  # 请求连接服务器
        pprint.pprint(self.client_socket.getpeercert())           # new 
    # 请求登录类型
    ...
    def send_string_with_length(self, content):
        ...
        # 将 content_str 编码为字节字符串
        content_bytes = content_str.encode('utf-8')

        # 发送内容的长度
        self.client_socket.sendall(len(content_bytes).to_bytes(4, byteorder='big'))
