...
import ssl     #new
certfile = "E:\\third\\Digital authentication\\test\\pythonProject-main\\2\\pythonProject-main\\python_chat\\server.crt"  #new
keyfile = "E:\\third\\Digital authentication\\test\\pythonProject-main\\2\\pythonProject-main\\python_chat\\server.key"   #new
...
if __name__ == "__main__":
    try:
        context = ssl.SSLContext(ssl.PROTOCOL_TLS)    # new
        # 加载私钥及对应的证书
        context.load_cert_chain(certfile=certfile, keyfile=keyfile)      # new   
        # 创建接受客户端连接的socket
        ...
        while True:
            ...
            # 包装一个现有的 Python socket，并返回一个 ssl socket，server_side 为 True 表示为服务器行为，默认为 False 表示客户端
            ssl_connect_sock = context.wrap_socket(connection, server_side=True)  # new
            Thread(target=handle, args=(ssl_connect_sock, address)).start()       # new
    except Exception as e:
        print("服务器出错: " + str(e))
